  ...
  return (
    <>
      <TodayOnlineTimeSheet
        ...
        minsPart={minsPart}
      />
      <div className="w-full overflow-hidden rounded-xl border border-[var(--color-gray-200)]/80 bg-white text-left shadow-[0_4px_20px_rgba(0,0,0,0.08)]">
        <button
          type="button"
          onClick={onOpenWallet}
          className={`${hitTarget} px-3 pb-2 pt-2.5`}
          aria-label={t("dashboard.summaryWalletSectionAria")}
        >
          <div className="flex items-baseline justify-between gap-2">
            <h2 className="text-sm font-bold text-[var(--color-primary)]">{t("dashboard.todaySummary")}</h2>
            <time className="text-[10px] font-medium text-[var(--color-text-secondary)]" dateTime={now.toISOString()}>
          ...
          <p className="mt-0.5 text-[9px] font-medium text-[var(--color-primary)]">{t("dashboard.walletTapHint")}</p>
        </button>

        <div className="grid grid-cols-3 gap-0 border-t border-[var(--color-gray-100)] px-0.5 py-1">
          <button
            type="button"
            onClick={onOpenWallet}
            className={`${hitTarget} flex-col items-center px-1 py-1.5`}
            aria-label={t("dashboard.summaryEarningsAria")}
          >
            <p className="text-[9px] font-semibold uppercase tracking-wide text-[var(--color-text-secondary)]">
              {t("dashboard.earnings")}
            </p>
            <p className="mt-0.5 text-sm font-bold tabular-nums text-[var(--color-primary)]">{formatInr(todayEarnings)}</p>
          </button>
          <button
            type="button"
            onClick={onOpenTripHistory}
            className={`${hitTarget} flex-col items-center border-x border-[var(--color-gray-200)] px-1 py-1.5`}
            aria-label={t("dashboard.summaryTripsAria")}
          >
            <p className="text-[9px] font-semibold uppercase tracking-wide text-[var(--color-text-secondary)]">
              {t("dashboard.trips")}
            </p>
            <p className="mt-0.5 text-sm font-bold tabular-nums text-[var(--color-primary)]">{completedTrips}</p>
          </button>
          <button
            type="button"
        ...
        <div className="space-y-1.5 border-t border-[var(--color-gray-100)] px-3 pb-2.5 pt-2">
          <button
            type="button"
            onClick={onOpenPartnerTier}
            className={`${hitTarget} w-full space-y-1 p-1.5`}
            aria-label={t("dashboard.summaryTargetAria")}
          >
            <div>
              <div className="flex justify-between text-[9px] font-bold uppercase tracking-wide text-[var(--color-text-secondary)]">
                <span>{t("dashboard.targetTitle")}</span>
                <span>{t("dashboard.targetSub", { done: completedTrips, goal: DAILY_TRIP_GOAL })}</span>
              </div>
              <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-[var(--color-gray-200)]">
                <motion.div
          ...
          <button
            type="button"
            onClick={onOpenPartnerTier}
            className={`${hitTarget} flex w-full items-center justify-between rounded-lg bg-[var(--color-gray-50)] px-2.5 py-1.5 hover:bg-[var(--color-gray-100)]/80`}
            aria-label={t("dashboard.summaryRatingAria")}
          >
            <span className="text-[11px] font-semibold text-[var(--color-text-secondary)]">{t("dashboard.ratingLabel")}</span>
            <span className="text-xs font-bold text-amber-600">{t("dashboard.ratingValue", { stars: "4.8" })}</span>
          </button>
        </div>
      </div>
    </>
