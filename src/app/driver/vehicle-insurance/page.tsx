...
import {
  ...
  insurancePolicyNumberSchema,
  vehicleInsuranceExtractedSchema,
} from "@/lib/driver/vehicleInsuranceValidation";

const MAX_BYTES = 10 * 1024 * 1024;
const OCR_DELAY_MS = 900;
const VERIFY_MS = 1400;

const MOCK_POLICY = "POL-MH-2024-558812";
const MOCK_EXPIRY = "22-Apr-2026";
...
function VehicleInsuranceContent() {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<"file" | "policyNumber" | "expiryDate", string>>
  >({});
  ...
  const policyValid = insurancePolicyNumberSchema.safeParse(policyNumber.trim()).success;
  const expiryValid = insuranceExpiryDateSchema.safeParse(expiryDate.trim()).success;
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!file) next.file = "Upload your insurance document.";
    const parsed = vehicleInsuranceExtractedSchema.safeParse({
      policyNumber: policyNumber.trim(),
      expiryDate: expiryDate.trim(),
    });
    if (!parsed.success) {
      for (const issue of parsed.error.issues) {
        const k = issue.path[0] as "policyNumber" | "expiryDate";
    ...
    if (Object.keys(next).length > 0) {
      ...
      const scrollId =
        first === "file" ? "ins-file" : first === "policyNumber" ? "ins-policy" : "ins-expiry";
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-white">
      <OnboardingStepHeader step={5} totalSteps={8} percent={50} />

      <div className="flex-1 overflow-y-auto px-5 pb-4 pt-5">
        <h1 className="text-xl font-bold text-[var(--color-text-primary)]">Upload vehicle insurance</h1>
        <p className="mt-1 text-[13px] leading-relaxed text-[var(--color-text-secondary)]">
          Please upload a clear photo of your valid vehicle insurance document.
        </p>
        ...
        >
          <div className="flex flex-col items-center text-center">
            ...
            <div className="mt-5 flex w-full max-w-[280px] gap-3">
              ...
              >
                <CameraIcon className="size-5 text-[var(--color-text-secondary)]" />
                Camera
              </button>
              <button
                type="button"
                ...
                className="flex flex-1 items-center justify-center gap-2 rounded-[var(--radius-standard)] border border-[var(--color-gray-200)] bg-white py-3 text-sm font-semibold text-[var(--color-text-primary)] shadow-sm"
              >
                <ImageGalleryIcon className="text-[var(--color-text-secondary)]" />
                Gallery
              </button>
            </div>
          </div>
        ...
        <div className="mt-6 rounded-[var(--radius-standard)] border border-[var(--color-gray-200)] bg-white p-4 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="ins-policy" className="mb-1 block text-xs font-medium text-[var(--color-text-secondary)]">
                Policy Number
              </label>
              <div className={inputRow}>
                <input
                  ...
                  autoComplete="off"
                />
                {policyValid ? <FieldVerifiedIcon className="shrink-0" /> : <span className="w-[22px] shrink-0" />}
              </div>
              {errors.policyNumber ? (
                <p className="mt-1 text-xs text-[var(--color-error)]">{errors.policyNumber}</p>
              ) : null}
            </div>

            <div>
              <label htmlFor="ins-expiry" className="mb-1 block text-xs font-medium text-[var(--color-text-secondary)]">
          ...
          {ocrDone ? (
            <div className="mt-4 flex items-center gap-2 text-xs text-[var(--color-text-secondary)]">
              ...
              <span>{verifying ? "Verifying policy details…" : "Verified with insurer records."}</span>
            </div>
          ) : null}
        ...
        <div className="mt-5 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <p className="text-xs leading-relaxed text-amber-900">
            <span className="font-semibold">Note:</span> Your insurance expires in 15 days. Please ensure it is renewed
            soon to avoid service interruption.
          </p>
        </div>
      </div>
